...
export default function RootLayout() {
  ...
  // 2. ESTADO PARA LA ANIMACIÓN LOTTIE
  const [splashAnimationFinished, setSplashAnimationFinished] = useState(false);

  // 3. CONFIGURACIÓN DE TOAST (NOTIFICACIONES)
  const toastConfig = {
    success: (props: any) => (
      <BaseToast
        {...props}
        ...
        text1Style={{
          ...
          fontWeight: "900",
          color: "#fff",
          fontFamily: "Helvetica", // Asegúrate de que este nombre coincida con tu fuente cargada
        }}
        text2Style={{
          fontSize: 13,
          color: "#D8B4FE",
  ...
  // Efecto para la barra de navegación de Android
  useEffect(() => {
  ...
  // 4. LÓGICA DE OCULTADO DEL SPLASH NATIVO
  const onLayoutRootView = useCallback(async () => {
    if (loaded) {
      // Ocultamos la imagen estática (PNG) apenas carguen las fuentes.
      // Como <AnimatedSplash> está renderizado encima (si !finished), el usuario verá la transición suave.
  ...
  return (
    <ThemeProvider value={colorScheme === "dark" ? DarkTheme : DefaultTheme}>
      ...
      {/* Toast debe estar fuera de la View principal para flotar sobre el Splash si fuera necesario */}
